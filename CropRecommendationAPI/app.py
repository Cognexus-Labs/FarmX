...
"""
Todo: Request no longer supports request.get_json(force=True) change to something else
"""
...
def easy_predict():
    ...
    coordinates = (float(request.form.get('lat')), float(request.form.get('lon')))#(data['lat'], data['lon'])
    print(coordinates)
