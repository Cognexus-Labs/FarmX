...
class easy_parameter_finders:
    ...
    def get_weather_forecast(self, coordinates, all=False):
        lat, lon = coordinates
        url = "https://api.openweathermap.org/data/2.5/forecast?lat={}&lon={}&appid={}".format(lat,lon,Open_weathermaps_api_key)
        response = requests.request("GET", url, headers={}, data={})
        if all:
            return(json.loads(response.text))
        else:
    ...
    def dump_all_data(self, coordinates):
        dump = {
        "#### forecast ####":(self.get_weather_forecast(coordinates, all=True)),
        "#### forecast_docs ####": " for meaning of these params, check the docs here bruv - https://openweathermap.org/forecast5", 
